...
// ===== Blinn-Phong Shading (per-pixel in raster) =====
...
// ===== Triangle Rasterization (Phong shading per-pixel) =====
void raster_triangle_phong(const VScreen& a, const VScreen& b, const VScreen& c) {
    ...
    for (int y = minY; y <= maxY; ++y)
        for (int x = minX; x <= maxX; ++x) {
            ...
            Vec3 interp_cam = a.cam * aB + b.cam * bB + c.cam * cB;
            Vec3 interp_nrm = normalize(a.normal * aB + b.normal * bB + c.normal * cB);
            Vec3 color = shade(interp_cam, interp_nrm);
            put_pixel(x, y, depth, color);
        }
}
...
void render() {
    ...
    for (int i = 0; i < gNumVertices; ++i) {
        ...
        Vec3 cam = obj * 2.f + Vec3{ 0, 0, -7.f };  
        Vec3 normal = normalize(obj);      
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(Width, Height, "Phong Shaded Sphere", NULL, NULL);
